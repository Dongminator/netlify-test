...
- 到场签到：已报名会员点击"📍 到场签到"，浏览器获取手机GPS位置，服务端校验与球场坐标（`data/db.json` 中每个活动的 `coords`）的距离 ≤ 10米（`server.js` 中 `CHECKIN_RADIUS_METERS`）才签到成功；名单中显示"已签到"标记，取消报名会同时清除签到
- `/member-list` 会员列表
...
另外手机GPS精度通常为5~20米，10米阈值在实际使用中可能偏严，可调大 `CHECKIN_RADIUS_METERS`。
